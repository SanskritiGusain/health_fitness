...
class WaistMeasurementScreen extends StatefulWidget {
  const WaistMeasurementScreen({super.key});

  @override
  State<WaistMeasurementScreen> createState() => _WaistMeasurementScreenState();
}

class _WaistMeasurementScreenState extends State<WaistMeasurementScreen> {
  bool isCm = true; // true = cm, false = in
  int selectedWaistCm = 35;
  double selectedWaistIn = 13.8;
  ...
  void initState() {
    super.initState();
    _controllerCm = FixedExtentScrollController(initialItem: selectedWaistCm - 30);
    _controllerIn = FixedExtentScrollController(initialItem: ((selectedWaistIn * 10).round() - 118));
  }
  ...
  void _toggleUnit() {
    setState(() {
      ...
      if (isCm) {
        ...
        selectedWaistCm = (selectedWaistIn * 2.54).round();
        selectedWaistCm = selectedWaistCm.clamp(30, 50);
        ...
        selectedWaistIn = ( selectedWaistCm / 2.54);
      selectedWaistIn =  selectedWaistIn.clamp(11.8, 19.8);
        _controllerIn.animateToItem(
          (( selectedWaistIn * 10).round() - 118),
          duration: const Duration(milliseconds: 300),
          curve: Curves.easeInOut,
        );
      }
    ...
    const SizedBox(height: 20),
          ...
          Text.rich(
            TextSpan(
              children: [
                TextSpan(
                  text: isCm ? '$selectedWaistCm' : '${selectedWaistIn.toStringAsFixed(1)}',
                    ...
                    child: RotatedBox(
                      ...
                      child: ListWheelScrollView.useDelegate(
                        ...
                        onSelectedItemChanged: (index) {
                          setState(() {
                            if (isCm) {
                              selectedWaistCm = cmValues[index];
                            } else {
                              selectedWaistIn = inValues[index];
                            }
                          });
                        },
              ...
              children: [
                const Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 10),
                    Text(
                      'Track waist measurement progress',
                      style: TextStyle(
                        fontSize: 12,
