...
class BodyNeckScreen extends StatefulWidget {
  const BodyNeckScreen({super.key});

  @override
  State<BodyNeckScreen> createState() => _BodyNeckScreenState();
}

class _BodyNeckScreenState extends State<BodyNeckScreen> {
  bool isCm = true; // true = cm, false = in
  int selectedNeckCm = 35;
  double selectedNeckIn = 13.8;
  ...
  void initState() {
    super.initState();
    _controllerCm = FixedExtentScrollController(initialItem: selectedNeckCm - 30);
    _controllerIn = FixedExtentScrollController(initialItem: ((selectedNeckIn * 10).round() - 118));
  }
  ...
  void _toggleUnit() {
    setState(() {
      ...
      if (isCm) {
        ...
        selectedNeckCm = (selectedNeckIn * 2.54).round();
        selectedNeckCm = selectedNeckCm.clamp(30, 50);
        ...
        selectedNeckIn = (selectedNeckCm / 2.54);
        selectedNeckIn = selectedNeckIn.clamp(11.8, 19.8);
        _controllerIn.animateToItem(
          ((selectedNeckIn * 10).round() - 118),
          duration: const Duration(milliseconds: 300),
          curve: Curves.easeInOut,
        );
      }
    ...
    const SizedBox(height: 20),
          ...
          Text.rich(
            TextSpan(
              children: [
                TextSpan(
                  text: isCm ? '$selectedNeckCm' : '${selectedNeckIn.toStringAsFixed(1)}',
                    ...
                    child: RotatedBox(
                      ...
                      child: ListWheelScrollView.useDelegate(
                        ...
                        onSelectedItemChanged: (index) {
                          setState(() {
                            if (isCm) {
                              selectedNeckCm = cmValues[index];
                            } else {
                              selectedNeckIn = inValues[index];
                            }
                          });
                        },
              ...
              children: [
                const Column(
                  ...
                  children: [
                    ...
                    SizedBox(height: 10),
                    Text(
                      'Track neck measurement progress',
                      style: TextStyle(
                        fontSize: 12,
